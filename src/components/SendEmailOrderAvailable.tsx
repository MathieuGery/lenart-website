...
export default function SendEmailOrderAvailable({
  orderId,
  orderNumber,
  orderStatus,
  customerName,
  ...
  const isDisabled = (orderStatus !== 'order-available-at-desk' || !amazonLink) || isLoading
  ...
  async function handleSendEmail() {
    ...
    try {
      ...
      const response = await fetch(`/api/orders/${orderId}/send-email-order-available`, {
        method: 'POST',
        headers: {
  ...
  return (
    <div>
      ...
      >
        ...
        {isLoading ? 'Envoi en cours...' : 'Commande disponible au club'}
      </button>

      {!amazonLink || orderStatus !== 'order-available-at-desk' && (
        <p className="mt-2 text-xs text-gray-500">
          La commande doit contenir un lien amazon ainsi qu'être en status Photos Disponible au club
        </p>
      )}
    </div>
  )
